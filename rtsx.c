...
static int rtsx_acquire_irq(struct rtsx_dev *dev) {
  ...
  printk(KERN_INFO "%s: chip->msi_en = %d, pci->irq = %d\n", __FUNCTION__,
         chip->msi_en, dev->pci->irq);

  if (request_irq(dev->pci->irq, rtsx_interrupt, chip->msi_en ? 0 : IRQF_SHARED,
                  CR_DRIVER_NAME, dev)) {
...
static void rtsx_release_resources(struct rtsx_dev *dev) {
  ...
  if (dev->rtsx_resv_buf) {
    dma_free_coherent(&(dev->pci->dev), RTSX_RESV_BUF_LEN, dev->rtsx_resv_buf,
...
static int rtsx_probe(struct pci_dev *pci, const struct pci_device_id *pci_id) {
  ...
  dev->rtsx_resv_buf = dma_alloc_coherent(
      &(pci->dev), RTSX_RESV_BUF_LEN, &(dev->rtsx_resv_buf_addr), GFP_KERNEL);
...
module_init(rts5229_init) module_exit(rts5229_exit)
